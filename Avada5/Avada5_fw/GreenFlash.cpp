...
void EnterDelayBeforeRestart() {
    ...
    ITmr.StartOrRestart(TIME_MS2I(Settings.Delay.BeforeRestart.Value));
}
...
void EnterReady() {
    ...
    ITmr.StartOrRestart(TIME_MS2I(Settings.Delay.Ready2Press.Value));
}
...
void Init() {
    ...
    DAC->DHR12R1 = 0;
    // Set initial state
    if(Settings.Delay.Start2Ready.Value == 0) {
        Buzzer.BeReady();
        EnterReady();
    }
    else EnterCharging();
...
//    Printf("DE: %u\r", State);
    switch(State) {
        ...
        case staReady:
            ...
            if(Settings.Delay.OffIfNotFired.Value != 0) ITmr.StartOrRestart(TIME_MS2I(Settings.Delay.OffIfNotFired.Value));
            break;
...
void OnBtnPress() {
    if(State == staWaitingBtnPress) {
        ...
        ITmr.StartOrRestart(TIME_MS2I(Settings.Delay.BeforeFlash.Value));
    }
}
